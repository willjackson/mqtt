...
class MqttBrokerForm extends ContentEntityForm {
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($status) {
      case SAVED_NEW:
        $this->messenger()->addMessage($this->t('Created the %label MQTT Broker.', [
          '%label' => $entity->label(),
        ]));
        break;

      default:
        $this->messenger()->addMessage($this->t('Saved the %label MQTT Broker.', [
          '%label' => $entity->label(),
        ]));
    }
    $form_state->setRedirect('entity.mqtt_broker.canonical', ['mqtt_broker' => $entity->id()]);
  }
